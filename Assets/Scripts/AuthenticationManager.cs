...
public class AuthenticationManager : MonoBehaviour
{
    ...
    // In production, should probably keep these in a config file
    const string IdentityPool = "eu-central-1:a4a1fcd1-c7c4-4bc1-a1be-6872ee768ca5"; //insert your Cognito User Pool ID, found under General Settings
    const string AppClientID = "73nft57i2v16c9ilmf843aump0"; //insert App client ID, found under App Client Settings
    const string UserPoolId = "eu-central-1_PpZfEUis8";
    const string BucketName = "3dart-file-upload-test";
    ...
    public async Task<bool> Signup(string username, string email, string password)
    {
        ...
        List<Amazon.CognitoIdentityProvider.Model.AttributeType> attributes = new List<Amazon.CognitoIdentityProvider.Model.AttributeType>()
      {
        ...
        try
        {
            Amazon.CognitoIdentityProvider.Model.SignUpResponse sighupResponse = await _provider.SignUpAsync(signUpRequest);
    ...
    private async Task<string> GetUserIdFromProvider(string accessToken)
    {
        ...
        Task<Amazon.CognitoIdentityProvider.Model.GetUserResponse> responseTask =
           _provider.GetUserAsync(new Amazon.CognitoIdentityProvider.Model.GetUserRequest
           {
               AccessToken = accessToken
           });

        Amazon.CognitoIdentityProvider.Model.GetUserResponse responseObject = await responseTask;
    ...
    public async Task<bool> ConfirmSignupAsync(string code, string email)
    {
        ...
        //  var response = await _user.ConfirmSignUpAsync(signUpRequest);
        ...
        try
        {
            await user.ConfirmSignUpAsync(code, false);
            Debug.Log("Complete add user code");
            return true;
        }
        catch (Exception e)
        {
